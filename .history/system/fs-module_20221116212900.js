...
// a unique non blocking 
readFile(absolutePath+allFiles[0], 'utf-8', (err, result) => {
    if(err) {
        console.log(err);
        return 
    } 
    const first = allFiles[0] = result

    readFile(absolutePath+allFiles[1], 'utf-8', (err, result) =>{
        if(err){
            console.log(err);
        }
        const second = allFiles[1] = result
    })
    writeAFile()
})
